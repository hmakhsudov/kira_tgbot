...
TOKEN = '7033148059:AAEsKlWZHav6gbmpLqHVlBSCzb6_x7MXwEE'
bot = Bot(token=TOKEN)
dp = Dispatcher(bot)
...
async def callback_cook(callback_query: types.CallbackQuery):
    ...
    db.save_choice(True, recipe.id, callback_query.from_user.id)
    await bot.send_message(callback_query.message.chat.id, f"Отлично, вы приготовили {recipe.name}!")
    await callback_query.message.delete()
